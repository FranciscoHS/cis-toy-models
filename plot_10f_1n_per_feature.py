"""
Per-feature MSE (given feature active) for the F=10, n=1 models, sorted.
"""
...
if __name__ == "__main__":
    ...
    ax.plot(np.sort(err_l2), "s-", color="C1", markersize=7, label="L2-trained")
    ax.plot(np.sort(err_l4), "o-", color="C0", markersize=7, label="L4-trained")
    ...
    ax.set_title(f"Per-feature error: L2 vs L4  ({F} features, {N} neuron, p={P})")
    ax.legend(fontsize=9, loc="best")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()

    out_path = FIGURES_DIR / "l2_vs_l4_per_feature_10f_1n.png"
